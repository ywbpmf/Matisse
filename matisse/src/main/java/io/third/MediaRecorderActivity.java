...
public class MediaRecorderActivity extends Activity implements
     ...
     * 回删按钮、延时按钮、滤镜按钮
     */
    private CheckedTextView mRecordComplete;
    /**
     * 闪光灯
     */
    ...
    public static void goSmallVideoRecorder(Activity context, MediaRecorderConfig mediaRecorderConfig) {
        context.startActivityForResult(new Intent(context, MediaRecorderActivity.class).putExtra(MEDIA_RECORDER_CONFIG_KEY, mediaRecorderConfig), 1111);
    }
    ...
    private void loadViews() {
        ...
        mRecordComplete = findViewById(R.id.record_complete);
...
//        mRecordDelete.setOnClickListener(this);
    ...
    private void initSurfaceView() {
        if (NEED_FULL_SCREEN) {
            ...
            int margin = (int) (w / (MediaRecorderBase.SMALL_VIDEO_HEIGHT / (MediaRecorderBase.SMALL_VIDEO_WIDTH * 1.0f)));
            ((RelativeLayout.LayoutParams) mBottomLayout.getLayoutParams()).topMargin = margin;
            int width = w;
            int height = (int) (w * ((MediaRecorderBase.mSupportedPreviewWidth * 1.0f) / MediaRecorderBase.SMALL_VIDEO_HEIGHT));

//            LogUtils.d("margin " + margin + "w " + w + "height " + height);
            //
            FrameLayout.LayoutParams lp = (FrameLayout.LayoutParams) mSurfaceView
                    .getLayoutParams();
            lp.width = width;
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (!startState) {
                        ...
                        mMediaObject.buildMediaPart(mMediaRecorder.isFrontCamera() ? Camera.CameraInfo.CAMERA_FACING_FRONT : Camera.CameraInfo.CAMERA_FACING_BACK);
...
//                        mRecordDelete.setChecked(false);
        ...
        } else if (id == R.id.title_next || id == R.id.record_complete) {// 停止录制
            stopRecord();
            /*finish();
...
//                    mRecordDelete.setVisibility(View.GONE);
    ...
    public void onEncodeComplete() {
        ...
        Intent intent = getIntent();
        intent.putExtra("url", mMediaObject.getOutputTempTranscodingVideoPath());
